...
def load_extensions(exts: Sequence[str | dict[str, Any] | Type[Extension]]) -> Extensions:  # noqa: WPS231
    ...
    for extension_item in exts:
        if issubclass(extension_item, Extension):  # type: ignore[arg-type]
            extensions.add(extension_item)  # type: ignore[arg-type]
            continue
        ...
        if isinstance(extension_item, str):
            ...
            import_path = next(extension_item.keys())  # type: ignore[call-overload]
            if import_path in builtin_extensions:
                ext_module = builtin_extensions[import_path]
            else:
                ext_module = __import__(import_path)
            options = next(extension_item.values())  # type: ignore[call-overload]

        # TODO: handle AttributeError
        extension = ext_module.get_extension(**options)  # type: ignore[attr-defined]
