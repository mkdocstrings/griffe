...
def test_parse__module_attributes_section__expected_attributes_section(parse_sphinx: ParserType) -> None:
    ...
    for index, expected in enumerate(expected_kwargs):
        actual_attr = attr_section.value[index]
        expected_attr = DocstringAttribute(**expected)
        assert isinstance(actual_attr, type(expected_attr))
        assert actual_attr.name == expected_attr.name
        assert actual_attr.as_dict() == expected_attr.as_dict()
    assert not warnings
